...
`-identifyDevices` 參數
 a| 
SMCLI 中現在提供了一個新 `-identifyDevices`參數。此新參數可讓您尋找與儲存陣列相關聯的所有 SCSI 原生區塊裝置。如需更多資訊、請參閱 https://docs.netapp.com/us-en/e-series-cli/get-started/downloadable-smcli-parameters.html#identify-Devices["可下載的SMcli命令列參數"^]。
...
乙太網路核心統計資料
 a| 
系統管理員的「檢視 iSCSI 統計資料套件」頁面下方現在提供新的「乙太網路核心統計資料」選項。此新選項可讓您檢視 iSCSI 裝置平台核心驅動程式的統計資料。如需更多資訊、請參閱 https://docs.netapp.com/us-en/e-series-santricity/sm-support/view-iscsi-statistics-packages-support.html["檢視 iSCSI 統計資料套件"^]。
...
新增透過 REST API 端點封鎖 IP 位址的功能
 a| 
用戶現在可以通過設置端點來阻止特定的 IP 地址(`/devmgr/v2/settings`）。一旦透過「設定」端點進行設定、只有透過白名單指定的 IP 位址才能與儲存設備通訊。這項新功能支援 IPv4 和 IPv6 位址清單。
